...
### 1. Which stand should SK1425 use, and why?
Expected: re-stand to B16, High confidence (score 74). Schengen contact stand on the aircraft's own B pier, shortest walk, no baggage-belt change. Watch item: the buffer is 20 minutes, just below the 25-minute target. Duty manager confirms.
...
### 5. Why not put SK1425 on A12 instead?
Expected: A12 is feasible but worse than B16: a pier change to A, a longer walk, a baggage-belt change, and one size code larger than needed. It scores lower, so B16 wins.

### 6. SK1425 and LH2438 both want B16. How do you resolve it?
Expected: assign B16 to one of them; the other re-ranks automatically. If B16 goes to SK1425, LH2438 moves to A12 at Medium confidence. Two flights cannot hold one stand; the engine re-ranks the loser.
...
### 10. (Stress test) Put the A380 on B16.
Expected: refuse on constraint grounds. B16 is a code-C contact stand and the A380 is code F, so B16 is too small. The only stand that fits is R8. The agent should not comply with an illegal allocation.
